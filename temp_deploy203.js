...
const server = http.createServer(async (req, res) => {
  if (req.method === 'POST' && req.url === '/webhook/agent') {
    ...
    req.on('end', async () => {
      ...
      console.log('[*] ' + MY_ID + ' received from ' + msg.sender);
      ...
      const clawReq = http.request({ hostname: '127.0.0.1', port: 18789, path: '/v1/responses', method: 'POST', headers: { 'Authorization': 'Bearer ' + CLAW_TOKEN, 'Content-Type': 'application/json', 'x-openclaw-agent-id': 'main', 'Content-Length': Buffer.byteLength(clawBody) } }, (clawRes) => {
        ...
        clawRes.on('end', () => {
          if (msg.sender !== 'xinghuo') {
            ...
            const text = result.output && result.output[0] && result.output[0].content && result.output[0].content[0] ? result.output[0].content[0].text : 'done';
            const reply = JSON.stringify({ topic: msg.topic, sender: MY_ID, target: 'xinghuo', content: text });
            const r = http.request({ hostname: '192.168.10.195', port: 18080, path: '/relay', method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(reply) } }, () => {});
            r.on('error', () => {});
            r.write(reply);
            r.end();
          }
...
`;
  ...
  c.sftp((err, sftp) => {
    sftp.writeFile('/home/y2k1/relay_sidecar.js', script, (err) => {
      if (err) { console.log('Upload error:', err.message); c.end(); return; }
      console.log('Script uploaded to 203');
      c.end();
    });
  });
