...
const server = http.createServer(async (req, res) => {
  if (req.method === 'POST' && req.url === '/webhook/agent') {
    ...
    req.on('end', async () => {
      ...
      console.log('[*] ' + MY_ID + ' from ' + msg.sender + ': ' + msg.content.substring(0, 30));
...
c.on('ready', () => {
  c.sftp((err, sftp) => {
    sftp.writeFile('/home/y2k1/relay_sidecar.js', script, (err) => {
      ...
      // Start sidecar
      c.exec('cd /home/y2k1 && (nohup node relay_sidecar.js &) && sleep 3 && ps aux | grep relay_sidecar | grep -v grep', (err, stream) => {
        let out = '';
