...
`;
  ...
  c.sftp((err, sftp) => {
    sftp.writeFile('/home/y2k1/relay_sidecar.js', script, (err) => {
      ...
      console.log('Updated 203 sidecar');
      // Kill old and start new
      c.exec('pkill -f relay_sidecar; sleep 1; cd /home/y2k1 && nohup node relay_sidecar.js > relay_sidecar.log 2>&1 & sleep 2 && ps aux | grep relay_sidecar | grep -v grep', (err2, stream2) => {
        let out = '';
        stream2.on('data', d => out += d);
        stream2.on('end', () => { console.log(out); c.end(); });
      });
    });
  });
