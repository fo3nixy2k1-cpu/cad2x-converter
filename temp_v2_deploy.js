...
const fs = require('fs');
const c = new Client();
const script = fs.readFileSync('C:\\Users\\y2k1\\.openclaw\\workspace\\deploy_scripts\\sidecar_qiming_v2.js', 'utf8');

c.on('ready', () => {
  c.sftp((err, sftp) => {
    sftp.writeFile('/home/y2k1/relay_sidecar.js', script, (err) => {
      if (err) { console.log('Write error:', err.message); c.end(); return; }
      console.log('Uploaded v2');
      // Restart sidecar
      c.exec('fuser -k 18081/tcp 2>/dev/null; sleep 2; cd /home/y2k1 && nohup node relay_sidecar.js </dev/null >/dev/null 2>&1 &', (err2) => {});
      setTimeout(() => {
